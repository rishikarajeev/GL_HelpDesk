...
import home from "../Assets/Images/el_home-alt.png";
        ...
        <Nav className="flex-column ms-auto">
          <Nav.Item className={`nav-item mb-5 ${activeIcon === 'logo' ? 'selectedColor' : ''}`}>
            <Nav.Link className="nav-link" onClick={() => setActiveIcon('logo')} href="/">
              <img src={home_idon} alt="Home Icon" className="icon" />
            </Nav.Link>
          </Nav.Item>
          <Nav.Item className={`nav-item p-2 ${activeIcon === 'home' ? 'selectedColor' : ''}`}>
            <Nav.Link className="nav-link text-white" onClick={() => setActiveIcon('home')} href="/">
